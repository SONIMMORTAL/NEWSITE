            ...
            <FadeInSection>
                <div className="text-center max-w-3xl mx-auto mb-12">
                    ...
                    >
                        ...
                        <span className="text-sm text-brand font-medium">Community Resources</span>
                    </motion.div>
                ...
                <div className="space-y-4">
                    {PANTRY_LOCATIONS.map((pantry, i) => (
                        <FadeInSection key={i} delay={i * 0.1}>
                            <motion.div
                                className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 bg-card p-6 rounded-2xl border border-border shadow-lg"
                                whileHover={{ y: -2, boxShadow: '0 20px 40px rgba(0,0,0,0.1)' }}
                            >
                                <div>
                                    <h4 className="text-xl font-bold text-foreground mb-2">{pantry.name}</h4>
                                ...
                                <span className={`px-4 py-2 rounded-xl text-xs font-bold uppercase tracking-wider self-start sm:self-auto ${pantry.status === 'Open'
                                    ? 'bg-secondary text-brand border border-border'
                    ...
                    <FadeInSection variant="right" delay={0.2}>
                        <div className="bg-gradient-to-br from-yellow-400 via-yellow-500 to-amber-500 p-[2px] rounded-3xl shadow-2xl shadow-yellow-500/20">
                            <div className="bg-card rounded-[22px] overflow-hidden">
                                <div className="bg-gradient-to-br from-yellow-50 to-amber-50 p-6 border-b border-primary/30">
                                    <div className="flex items-center gap-3 mb-3">
                                        <div className="p-3 bg-gradient-to-br from-yellow-400 to-amber-500 text-white rounded-xl shadow-lg">
                                            <Sparkles size={20} />
                                        </div>
                                        <div>
                                            <h3 className="text-lg font-bold text-foreground">Pantry Chef AI</h3>
                                ...
                                {recipe && (
                                    ...
                                    >
                                        <div className="p-4 bg-gradient-to-br from-secondary to-emerald-50 border border-border rounded-xl text-sm text-foreground max-h-80 overflow-y-auto custom-scrollbar prose prose-sm prose-green prose-p:my-1 prose-headings:mb-2 prose-ul:my-1 prose-li:my-0">
